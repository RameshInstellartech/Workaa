...
- (void)setup {
	...
	_tokenField = [[TITokenField alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
...
//    NSLog(@"frame = %@",NSStringFromCGRect(_tokenField.frame));
...
- (void)tokenFieldFrameDidChange:(TITokenField *)field
{
    ...
    frame.size.height = field.frame.size.height;
    self.frame = frame;
    
    if ([_tokenField.delegate respondsToSelector:@selector(getframe:)])
    {
        [_tokenField.delegate getframe:self.frame.size.height];
    }
...
- (void)setup {
	...
	[self setFont:[UIFont fontWithName:@"Lato-Regular" size:12.0]];
    [self setBackgroundColor:[UIColor whiteColor]];
	[self setAutocorrectionType:UITextAutocorrectionTypeNo];
	[self setAutocapitalizationType:UITextAutocapitalizationTypeNone];
	[self setContentVerticalAlignment:UIControlContentVerticalAlignmentTop];
    self.textColor = [UIColor colorWithRed:123.0/255.0 green:123.0/255.0 blue:123.0/255.0 alpha:1.0];
...
- (CGFloat)layoutTokensInternal {
	...
	CGFloat lineHeight = ceilf(self.font.lineHeight) + topMargin + 10;
...
- (BOOL)getTintColorRed:(CGFloat *)red green:(CGFloat *)green blue:(CGFloat *)blue alpha:(CGFloat *)alpha withhighlight:(BOOL)highlight;
@end
...
- (instancetype)initWithTitle:(NSString *)aTitle representedObject:(id)object font:(UIFont *)aFont {
	...
	if ((self = [super init])){
		...
		_font = aFont;
//		_tintColor = [TIToken greenTintColor];
        _tintColor = [UIColor colorWithRed:200.0/255.0 green:200.0/255.0 blue:200.0/255.0 alpha:1.0];
		_textColor = [UIColor colorWithRed:123.0/255.0 green:123.0/255.0 blue:123.0/255.0 alpha:1.0];
...
- (void)drawRect:(CGRect)rect {
	...
	CGFloat blue = 1;
	CGFloat alpha = 1;
	[self getTintColorRed:&red green:&green blue:&blue alpha:&alpha withhighlight:drawHighlighted];
...
{    
    ...
    if(highlight)
    {
        UIColor *color = [UIColor colorWithRed:0.0/255.0 green:202.0/255.0 blue:252.0/255.0 alpha:1.0];
        colorSpaceModel = CGColorSpaceGetModel(CGColorGetColorSpace(color.CGColor));
        components = CGColorGetComponents(color.CGColor);
    }
    else
    {
        colorSpaceModel = CGColorSpaceGetModel(CGColorGetColorSpace(_tintColor.CGColor));
        components = CGColorGetComponents(_tintColor.CGColor);
    }
